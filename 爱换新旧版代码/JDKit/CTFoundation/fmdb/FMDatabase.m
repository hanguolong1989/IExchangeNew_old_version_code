...
- (BOOL)openWithserializedMode {
    ...
    int err;
    sqlite3_shutdown();//先关闭再打开
    err=sqlite3_config(SQLITE_CONFIG_SERIALIZED);//加了这一句使多线程数据库操作变成串行，除了加了这句，还得把_isExecutingStatement判断条件删掉，不然多线程读写返回结果为空
    if (err == SQLITE_OK) {
