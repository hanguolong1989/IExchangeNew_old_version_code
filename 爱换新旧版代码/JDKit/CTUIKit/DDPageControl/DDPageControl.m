...
#define kDotSpace		30.0f //原始默认值为12.0f
...
- (void)drawRect:(CGRect)rect 
{
	...
	for (int i = 0 ; i < numberOfPages ; i++)
	{
		...
		if (i == currentPage)
		{
            //add type == DDPageControlTypeLeftFullRightFull
			if (type == DDPageControlTypeOnFullOffFull || type == DDPageControlTypeOnFullOffEmpty || type == DDPageControlTypeLeftFullRightFull)
			{
		...
		{
            if (type == DDPageControlTypeLeftFullRightFull)
            {
                CGContextSetFillColorWithColor(context, onColorCG) ;
				CGContextFillEllipseInRect(context, CGRectInset(dotRect, -0.5f, -0.5f)) ;
            }
...
//        CGContextSetStrokeColorWithColor(context, offColorCG);
//    }
//    CGContextMoveToPoint(context, CGRectGetMidX(currentBounds) - dotsWidth / 2 + diameter, y + 2);
//    if (currentPage > 0) {
//        CGContextAddLineToPoint(context, CGRectGetMidX(currentBounds) - dotsWidth / 2 + diameter + currentPage*(diameter + space), y+2);
//        CGContextStrokePath(context);
//    }
//    if (currentPage < self.numberOfPages - 1) {
//        CGContextSetStrokeColorWithColor(context, offColorCG);
//        CGContextMoveToPoint(context, CGRectGetMidX(currentBounds) - dotsWidth / 2 + diameter + currentPage*(diameter + space), y + 2);
//        CGContextAddLineToPoint(context, x - diameter - space, y + 2);
//        CGContextStrokePath(context);
//    }
...
- (void)setFrame:(CGRect)aFrame
{
    ...
    for (NSLayoutConstraint *constraint in self.constraints) {
        if (NSLayoutAttributeWidth == constraint.firstAttribute) {
            constraint.constant = aFrame.size.width;
        } else if (NSLayoutAttributeHeight == constraint.firstAttribute) {
            constraint.constant = aFrame.size.height;
        }
    }
}
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount
{
	...
	return CGSizeMake(pageCount * diameter + (pageCount - 1) * space + 20.0f, MAX(10.0f, diameter + 4.0f)) ;
}
